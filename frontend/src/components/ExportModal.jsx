      ...
      <div className="max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <div className="grid max-h-[calc(92vh-160px)] overflow-y-auto lg:grid-cols-[260px_1fr]">
          <aside className="border-b border-slate-200 bg-slate-50 p-5 lg:border-b-0 lg:border-r">
            ...
            <p className="mt-6 text-[10px] font-bold uppercase tracking-wide text-slate-400">Hojas incluidas</p><div className="mt-2 space-y-1">{sheets.map((sheet, index) => <button key={sheet} type="button" onClick={() => setPreview(sheet)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition ${preview === sheet ? 'bg-[#173b69] text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}><span className={`grid h-6 w-6 place-items-center rounded-md text-[9px] font-bold ${preview === sheet ? 'bg-white/15' : 'bg-slate-200'}`}>{index + 1}</span><span className="text-xs font-semibold">{sheet}</span><Icon name="chevronRight" size={13} className="ml-auto opacity-60" /></button>)}</div>
